...
**Current state:** 420 drugs across 38 therapeutic classes, with 1,559 cited references and 858 ATC + 72 RxNorm + 6 ICD-11 + 4 LOINC ontology codes. Every entry is **◆ Verified** — cited and cross-checked across ≥2 authoritative sources — and 618 content units pass the Iron Rule 0 integrity gate (`scripts/verify.mjs`) on every push.
...
| Phase | What | Status |
|---|---|---|
| **0** | 8-primitive infrastructure + cited-compilation catalog (420 drugs, 38 classes, 1,559 cited references) + MCP data-plane server (stdio + HTTP) + full provenance surfaces | ✅ shipped |
...
| `/drugs` | 420 entries grouped by 38 therapeutic classes |
| `/drugs/<slug>` | Per-drug detail with sticky sidebar (trust stamp, signatures, ontology, mirror, drafting) |
| `/drugs/class/<slug>` | 38 therapeutic class browse pages with prev/next nav |
