...
- **Search facets**: "Has dosing for" species chips on `/search`, and species tags on every result; the API docs gain an "Open ↗" link on each live endpoint.
- **Section nav on phones**: the "On this page" row now sticks under the header on every screen size — a single swipeable row on phones that keeps the current section's chip in view. Home shows the six most recently updated entries; Escape clears the drug-list filter.
- **Body-weight helper on dose tables**: type a weight in kg and every row with a per-kg figure shows the multiplied amount next to its basis ("≈ 2.4–4.8 mg (0.1–0.2 mg/kg × 24 kg)"). Arithmetic on the first mg/kg in the row only, always shown with its source so it can be checked — not a prescription.
- **Section nav follows your scroll**: the "On this page" chip for the section you are reading is highlighted.
- **A real 404 page** with search and catalog links instead of a blank screen.
- **Drug list filters as you type**: a filter box on `/drugs` narrows the 420 cards (name, Thai name, brand, ATC) instantly and hides empty classes, with a live "N of 420" count. On phones the header is one row (Verify and About move to the footer and the ⌘K palette); the home page shows 12 class chips with an "All 38 classes" expander instead of a wall of chips.
